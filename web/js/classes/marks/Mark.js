                ...
                setTimeout(function () {
                    ...
                    if (theMark && theMark.canvas) {
                        ...
                        if (theMark.parentObject && theMark.parentObject.isLocator) {
                            ...
                            theMark.inConnectors.forEach(function (connector) {
                                connector.contract(false, false, true);
                            });

                            if (theMark.parentObject.selectedMark) {
                                theMark.parentObject.selectedMark = null;
                            }
...
//        if (LOG) console.log(theMark);
    ...
    this.sendElementsToBack = function () {
        var theMark = this;

        theMark.blink(0.02);

        if (theMark.iText) {
            sendToBack(theMark.iText);
        }
        sendToBack(theMark);
        ...
        if (theMark.parentObject && theMark.parentObject.isLocator && !theMark.parentObject.isCompressed && theMark.parentObject.selectedMark === theMark) {
            ...
            if (!theMark.xVisualPropertyOcluded) {
                sendToBack(theMark.xVisualProperty);
            }

            sendToBack(theMark.yVisualProperty);
        }

        if (!theMark.isCompressed) {
            theMark.visualProperties.forEach(function (visualProperty) {
                if (visualProperty.canvas) {
                    sendToBack(visualProperty);
                    visualProperty.inConnectors.forEach(function (inConnection) {
                        sendToBack(inConnection);
                    });
                    visualProperty.outConnectors.forEach(function (outConnection) {
                        sendToBack(outConnection);
                    });

                }
            });
            sendToBack(theMark.backgroundRect);
        }
    };
...
//                this.activateCopyingMode();
            ...
            'moving': function (options) {
                ...
                theMark.opacity = 0.75;
                theMark.backgroundRect.opacity = 0.75;
