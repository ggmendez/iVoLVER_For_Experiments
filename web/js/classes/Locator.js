var Locator = fabric.util.createClass(fabric.Rect, {
    type: 'Locator',
...
//            source.typeIcon.bringToFront();
    ...
    positionChild: function (property, value, theMark, shouldAnimate) {
        ...
        if (untransformedProperty === 'untransformedX') {
            ...
            newCoordinateValue = theCoordinate + theLocator.width / 2 - theMark.width / 2;

        } else if (untransformedProperty === 'untransformedY') {

            newCoordinateValue = -theCoordinate + theLocator.width / 2 - theMark.height / 2;
    ...
    _render: function (ctx) {
        ...
        ctx.beginPath();
        ctx.arc(0, 0, this.width / 2 / 2.5, 0, 2 * Math.PI);
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
        ctx.restore();
        ...
        ctx.moveTo(-(theLocator.width / 2 + d + theLocator.strokeWidth), 0);
        ctx.lineTo(-(theLocator.width / 2 - d), 0);

        // Right horizontal line
        ctx.moveTo(+(theLocator.width / 2 - d), 0);
        ctx.lineTo(+(theLocator.width / 2 + d + theLocator.strokeWidth), 0);

        // Top vertical line
        ctx.moveTo(0, -(theLocator.width / 2 + d + theLocator.strokeWidth));
        ctx.lineTo(0, -(theLocator.width / 2 - d));

        // Bottom vertical line
        ctx.moveTo(0, +(theLocator.width / 2 - d));
        ctx.lineTo(0, +(theLocator.width / 2 + d + theLocator.strokeWidth));
        ...
        ctx.beginPath();
        // positive x axis
        var x = theLocator.width / 2 + d + theLocator.strokeWidth;
        if (theLocator.maxUntransformedX > theLocator.width / 2) {
            ctx.moveTo(theLocator.width / 2 + d + theLocator.strokeWidth, 0);
        ...
        ctx.beginPath();
        // negative x axis
        if (theLocator.minUntransformedX < -theLocator.width / 2) {
            ctx.moveTo(-(theLocator.width / 2 + d + theLocator.strokeWidth), 0);
        ...
        ctx.beginPath();
        // positive y axis
        var y = -(theLocator.width / 2 + d + theLocator.strokeWidth);
        if (theLocator.minUntransformedY < -theLocator.width / 2) {
            ctx.moveTo(0, -(theLocator.width / 2 + d + theLocator.strokeWidth));
        ...
        ctx.beginPath();
        // negative y axis
        if (theLocator.maxUntransformedY > theLocator.width / 2) {
            ctx.moveTo(0, +(theLocator.width / 2 + d + theLocator.strokeWidth));
        ...
        if (!theLocator.isCompressed && theLocator.selectedMark) {
            ...
            var x = selectedMark.untransformedX - theLocator.width / 2 + selectedMark.getWidth() / 2;
            var y = selectedMark.untransformedY - theLocator.width / 2 + selectedMark.height / 2;
...
//               var x = widget.untransformedX < 0 ? widget.untransformedX - theLocator.width/2 : widget.untransformedX + theLocator.width/2 + widget.width / 2;

                    var x = widget.untransformedX < 0 ? widget.untransformedX - theLocator.width / 2 + widget.width / 2 : widget.untransformedX + theLocator.width / 2;
...
//                    if (LOG) console.log("(" + widget.untransformedX + "," + widget.untransformedY + ")");
                    ...
                    var y = widget.untransformedY < 0 ? widget.untransformedY + widget.height / 2 - theLocator.width / 2 : widget.untransformedY - theLocator.width / 2 + widget.height / 2;
        ...
        fabric.util.animate({
            ...
            onComplete: function () {
                ...
                if (theLocator.selectedMark) {
                    ...
                    if (!theLocator.selectedMark.xVisualPropertyOcluded) {
    ...
    compress: function (refreshCanvas) {
        ...
        theLocator.widgets.forEach(function (widget) {
            if (widget.isMark) {
                ...
                if (xVisualProperty.canvas) {

                    xVisualProperty.inConnectors.forEach(function (inConnector) {
                        inConnector.opacity = 0;
                    });
                    xVisualProperty.outConnectors.forEach(function (outConnector) {
                ...
                if (yVisualProperty.canvas) {

                    yVisualProperty.inConnectors.forEach(function (inConnector) {
                        inConnector.opacity = 0;
                    });
                    yVisualProperty.outConnectors.forEach(function (outConnector) {
                        outConnector.opacity = 0;
                    });
    ...
    positionCoordinateVisualProperties: function () {
        ...
        if (xAxisArrow) {
            if (theLocator.maxUntransformedX > 0) {
                ...
                xAxisArrow.untransformedX = theLocator.width / 2 + xAxisArrow.width / 2 + gap;
            }
            repositionWidget(theLocator, xAxisArrow);
        }
        ...
        if (yAxisArrow) {
            if (theLocator.minUntransformedY < 0) {
                ...
                yAxisArrow.untransformedY = -theLocator.width / 2 - yAxisArrow.height / 2 - gap;
            }
            repositionWidget(theLocator, yAxisArrow);
        }
...
//        child.bringToFront();
    ...
    remove: function () {
        ...
        theLocator.widgets.forEach(function (widget) {
            if (widget.isMark) {
                widget.xVisualProperty.disconnect(false, true);
                widget.yVisualProperty.disconnect(false, true);
            }
        });
        theLocator.outConnectors.forEach(function (outConnection) {
            outConnection.contract(true, false, true, fabric.util.ease['easeOutCubic']);
        });
        hideWithAnimation(theLocator, true, 550);
    },
...
//                if (theLocator.moving || theLocator.scaling) {
            ...
            'mousedown': function (option) {
                ...
                bringToFront(theLocator);
                if (!theLocator.isCompressed && selectedMark) {

                    if (selectedMark.isCompressed) {
                        bringToFront(selectedMark.xVisualProperty);
                    } else {
                        if (!selectedMark.xVisualPropertyOcluded) {
                            bringToFront(selectedMark.xVisualProperty);
                        }
                    }

                    bringToFront(selectedMark.yVisualProperty);
                }
            },
            'pressed': function (theEvent) {
                ...
                var newConnector = new Connector({source: theLocator, x2: theLocator.left, y2: theLocator.top, arrowColor: 'rgb(86, 86, 86)', strokeWidth: 2, undirected: true});
...
//    options.colorForStroke = 'rgb(86, 86, 86)';
    options.colorForStroke = 'rgb(255,255,255)';
//    options.stroke = 'rgb(86, 86, 86)';
//    options.stroke = 'rgba(86, 86, 86, 0.5)';
    options.stroke = 'rgb(255,255,255)';
...
//    }
    ...
    if (options.animateAtBirth) {
        animateBirth(newLocator, options.markAsSelected, 1, 1, false);
    }
