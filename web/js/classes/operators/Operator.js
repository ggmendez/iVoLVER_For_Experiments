var Operator = fabric.util.createClass(fabric.Circle, {
    ...
    remove: function (refreshCanvas, removeAfterDisconnection) {
        
        if (isUndefined(refreshCanvas)) {
            refreshCanvas = true;
        }
        
        if (isUndefined(removeAfterDisconnection)) {
            removeAfterDisconnection = true;
        }
        
        var theOperator = this;
        theOperator.inConnectors.forEach(function (inConnection) {
            inConnection.contract(false, false, true);
        });
        theOperator.outConnectors.forEach(function (outConnection) {
            outConnection.contract(true, false, true);
        });
        
        hideWithAnimation(theOperator, refreshCanvas, 600);        
