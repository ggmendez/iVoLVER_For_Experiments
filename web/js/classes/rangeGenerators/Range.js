var Range = fabric.util.createClass(fabric.Rect, {
    ...
    remove: function (refreshCanvas) {
        var theRange = this;
        if (isUndefined(refreshCanvas)) {
            refreshCanvas = true;
        }
        theRange.lowerLimit.remove(false);
        theRange.upperLimit.remove(false);
        theRange.outputPoint.disconnect(false, true);
        hideWithAnimation(theRange.playButton, false);
        hideWithAnimation(theRange.pauseButton, false);        
        hideWithAnimation(theRange, refreshCanvas, 600);
    },
