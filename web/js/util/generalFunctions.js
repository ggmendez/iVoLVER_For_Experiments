...
function saveCanvasAsImage() {
    ...
    var d = new Date();
    var df = d.getMonth() + '_' + d.getDate() + '_' + d.getYear() + '_' + (d.getHours() + 1) + '_' + d.getMinutes() + '_' + d.getSeconds() + '_' + d.getMilliseconds();
    var fileName = "iVoLVERCanvas_" + df + ".png";
    var multiplier = 3;

    var origWidth = canvas.getWidth(),
...
function handleDatafiles(files, button) {
    var file = files[0];
    var reader = new FileReader();
    reader.onload = (function (file) {
        return function (evt) {
            onDataFileReadComplete(evt, file);
            resetLoadingFileButton(button);
        };
    })(file);
    if (file) {
...
function resetLoadingFileButton(button) {
    $(button).val('')
}

function handleSVGFiles(files, asSingleMark, button) {
...
//        console.log(formatXml(SVGString));
        ...
        fabric.loadSVGFromString(SVGString, function (objects, options) {
            ...
            parentObject.remove = function () {
                parentObject.widgets.forEach(function (child) {
                    if (child.disconnect) {
                        child.disconnect(false, true);
                    } else {
                        child.remove();
                    }
                });
                hideWithAnimation(parentObject, true, 700);
            };
...
//                    canvas.add(object);
            ...
            parentObject.widgets.forEach(function (object) {
                object.nonRemovable = true;
            });
...
function handleImageFiles(files, button) {
...
function hideWithAnimation(object, refreshCanvas, duration) {
...
function sendToBack(object) {
    fabric.util.removeFromArray(canvas._objects, object);
    canvas._objects.unshift(object);
}
...
function sendObjectToBack() {
    ...
    if (object && object.sendElementsToBack) {
        object.sendElementsToBack();
        canvas.renderAll();
    }
}
